...
import { fetchState, initQuantSearch, renderResults } from '../src/lib/quant-search-dom';
...
describe('query handling', () => {
  ...
  it('needs two or more characters', () => {
...
describe('response handling and escaping', () => {
  ...
  it('falls back to the snippet, then to an untitled page', () => {
    const [, second, third] = parseResults(fixture);
  ...
  it('treats a malformed body as no results', () => {
    ...
    expect(parseResults({ results: [null, 3] })).toEqual([]);
  });

  it('writes the status text for every state', () => {
    expect(statusText({ kind: 'idle' })).toBe('');
    expect(statusText({ kind: 'too-short' })).toBe('Enter at least 2 characters to search.');
    expect(statusText({ kind: 'loading', query: 'x' })).toBe('Searching…');
    expect(statusText({ kind: 'results', query: 'li', results: parseResults(fixture).slice(0, 1) })).toBe(
      'Showing 1 result for “li”'
    );
    expect(statusText({ kind: 'results', query: 'li', results: parseResults(fixture) })).toBe(
...
const pageHtml = (attrs: string) => `<!doctype html><html><body>
  <div data-quant-search ${attrs}>
    ...
    <div data-search-results hidden><h2 tabindex="-1" data-search-heading>Search results</h2><div data-search-rows></div></div>
    <template data-search-template><div class="col-xxs-12"><div class="ct-snippet"><div class="ct-snippet__title"><a class="ct-snippet__title-link" href="/">Result title</a></div><div class="ct-paragraph ct-snippet__summary">Result summary</div></div></div></template>
  </div></body></html>`;
...
describe('rendering', () => {
  it('fills cloned Snippets with textContent only', () => {
    ...
    renderResults(rows, template, parseResults(fixture));
    ...
    expect(links[1]!.textContent).toBe('Bad & bold');
...
describe('initQuantSearch', () => {
  it('makes no request and shows the Callout when no site ID is set', async () => {
    ...
    expect(await initQuantSearch(root, fetchImpl, '?q=library')).toBeNull();
  ...
  it('makes no request for a one-character query', async () => {
    const root = mount('data-site-id="abc"');
    const fetchImpl = vi.fn();
    const state = await initQuantSearch(root, fetchImpl, '?q=a');
    expect(state).toEqual({ kind: 'too-short' });
    expect(fetchImpl).not.toHaveBeenCalled();
    expect(root.querySelector('[data-search-status]')!.textContent).toBe('Enter at least 2 characters to search.');
  });
  ...
  it('POSTs the query with the configured limit and renders the results', async () => {
    ...
    const state = await initQuantSearch(root, fetchImpl, '?q=library');
  ...
  it('shows the empty state for no results', async () => {
    const root = mount('data-site-id="abc"');
    const state = await initQuantSearch(root, () => okResponse({ results: [] }), '?q=zzz');
  ...
  it('shows the error state for an HTTP error or a network failure', async () => {
    ...
    expect(await fetchState(config, 'x', notOk)).toEqual({ kind: 'error', query: 'x' });
    expect(await fetchState(config, 'x', () => Promise.reject(new Error('offline')))).toEqual({
      kind: 'error',
      query: 'x',
    });

    const root = mount('data-site-id="abc"');
    await initQuantSearch(root, notOk, '?q=library');
