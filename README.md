...
- **Ten demo pages** ported from CivicTheme's default content module, and sample `events`,
  `news` and `publications` entries. `/community-engagement` is built but not in the primary
  menu, as on the CivicTheme Drupal demo site; add it to `src/content/navigation/` to link it.
...
| `settings`     | JSON   | `name`, `tagline`, `logoLight`, `logoDark`, `favicon`, `footerText`, `acknowledgement`, `social[]` (`{ platform, url }`), `theme`, `search` (see [Search](#search))                  |
