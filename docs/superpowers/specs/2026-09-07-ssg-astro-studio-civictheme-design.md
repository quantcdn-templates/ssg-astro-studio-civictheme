...
**Date:** 2026-09-07
**Status:** Implemented (Task 19, 2026-09-09) — see "Implementation notes" at the end of this
document for deviations from the plan below.
...
│   │   │   ├── variables.base.scss       # theming entry point (sibling of 00-base/)
│   │   │   ├── variables.components.scss # theming entry point (sibling of 00-base/)
│   │   │   └── {00-base,01-atoms,02-molecules,03-organisms,04-templates}/
...
- Theming entry points: `src/civictheme/scss/variables.base.scss` and
  `variables.components.scss` (siblings of `00-base/` inside `scss/`, not one level higher —
  the files `00-base/variables.scss` imports via `@import '../variables.base'`). Shipped
  populated with the CivicTheme default palette and a
...
Recorded at Task 19 (final task before merge). These are the deviations between this design
and the built template; each is a deliberate call made during implementation, not a gap.

- **Component count is 77, not 91.** 7 base, 22 atoms, 30 molecules, 18 organisms. The "91"
  above (Decisions table, Section 2, Section 7, Risks) counted Storybook-only
  `*.stories.twig` files, which are not components and are not ported (R1 of the shared
  reference). Parity case count is 697, not "~300" — every upstream Jest case got a case, not
  a sample.
- **Story parity added as a second oracle**, at Stuart's request, beyond the Jest-snapshot
  harness in Section 2/7: `npm run test:story-parity` (HTML, 171 stories) and
  `npm run test:story-parity:visual` (pixel, 165 measured — 6 of the 171 are unmeasurable, see
  `PARITY.md`). Fixtures are captured once from upstream Storybook into
  `tests/story-parity/fixtures/` so the suite runs without an upstream checkout. Not in the
  original design; `PARITY.md` (generated by `scripts/story-parity-report.mjs`) is the
  detailed report for both oracles.
- **No `<ClientRouter>`** (Astro's view-transitions router) — not mentioned in Section 5/6.
  CivicTheme's vendored behaviours self-initialise at module evaluation with no re-init guard
  on client-side navigation, so enabling it would silently break `tabs.js` and
  `skip-to-target.js` on a second visit to a page. Every navigation is a full page load
  instead. See `PORTING.md` and the "Behaviours" section of README.
- **Slots with string-prop fallback**: Section 2 says "Twig slots → Astro named slots"; in
  practice every Slot-documented prop is BOTH an Astro named slot AND keeps its string prop as
  a fallback, so MDX authors can pass plain text/Markdown without Astro slot syntax. A
  conditional `<Fragment slot="…">` still registers the slot at compile time even when its
  branch is false, which widens some wrapper gates — see `PageBanner.astro` and `PORTING.md`.
- **Pager URLs**: paginated listings serve page 1 at `/<collection>` and later pages at
  `/<collection>/page/N` (not `/<collection>/2`), so a pager page can never collide with a
  detail route whose slug is a bare number. Not specified in Section 5's routes list.
- **Twig.js oracle, not PHP Twig**: the Jest-snapshot parity harness (Section 2) runs upstream
  component tests through Twig.js (the same engine upstream's own Storybook/Jest setup uses),
  not server-side PHP Twig. Two Twig.js quirks are reproduced rather than "fixed" so the port
  stays byte-faithful to the oracle it is actually checked against: `x ? x : y` evaluates to
  `y` (not `x`) when `x` is undefined, and `undefined == false` is `true`. Both are recorded in
  `PORTING.md`.
